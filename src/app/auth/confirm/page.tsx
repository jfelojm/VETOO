  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      if (authData.session) {
        ...
        // Detectar si es staff o owner
        const rol = authData.session.user.user_metadata?.rol
        const barberoId = authData.session.user.user_metadata?.barbero_id
        setTimeout(() => {
          if (rol === 'barbero' && barberoId) {
            window.location.replace('/barbero/dashboard')
          } else {
